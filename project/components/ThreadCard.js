...
export function ThreadCard({ thread, onClick, active = false }) {
  return (
    <Link href={`/message/${thread.id}`}>
      <a
        onClick={onClick}
        className={`p-2.5 space-x-2 w-full rounded-lg flex items-center ${active ? "bg-gray-800" : ""}`}
      >
        ...
        <div>
          ...
          <div className="text-sm text-gray-400">{formatDistanceToNow(new Date(thread.created_at))}</div>
        </div>
      </a>
    </Link>
  );
