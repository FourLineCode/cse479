...
export default async function handler(req, res) {
  ...
  const getFriendshipQuery = `
    ...
    LIMIT 1
    `;
  const [friendship] = await query(getFriendshipQuery, [curr_id, curr_id, user_id, user_id]);
  if (friendship) {
  ...
  const getRequestQuery = `
    ...
    LIMIT 1
    `;
  const [request] = await query(getRequestQuery, [curr_id, curr_id, user_id, user_id]);
  if (request) {
