...
export default async function handler(req, res) {
  try {
    ...
    const existingUsers = await query(doesUserExistQuery, [input.email, input.username]);
    ...
    await query(createUserQuery, [input.email, input.username, input.password, input.bio]);
